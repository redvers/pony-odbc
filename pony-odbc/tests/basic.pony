...
class \nodoc\ iso _MariaDBBasic is UnitTest
  ...
  fun simple_test(h: TestHelper, dbh: ODBCDbc) ? =>
     ...
     */
    sth.directexec("CREATE TEMPORARY TABLE smpltest (a int, b varchar(25))")?
    sth.directexec("INSERT INTO smpltest VALUES (1, 'Row no 1')")?
    sth.directexec("INSERT INTO smpltest VALUES (2, 'Row no 2')")?
  ...
  fun simple_test1(h: TestHelper, dbh: ODBCDbc) ? =>
    ...
    sth.directexec("CREATE TEMPORARY TABLE t_basic (id INT PRIMARY KEY, name VARCHAR(20))")?
    sth.directexec("INSERT INTO t_basic VALUES (1,'foo'),(2,'bar'),(3,'baz')")?
    sth.directexec("UPDATE t_basic SET name = 'bop' WHERE id = 2")?

    h.assert_eq[I64](1, sth.rowcount()?)

    sth.directexec("DELETE FROM t_basic WHERE id = 3")?
    h.assert_eq[I64](1, sth.rowcount()?)

    sth.directexec("ALTER TABLE t_basic RENAME TO t_basic_2")?
    try
      sth.directexec("DROP TABLE t_basic")?
      h.fail("This drop should have failed")
    else
      h.assert_true(true)
    end
    sth.directexec("DROP TABLE t_basic_2")?
  ...
  fun select1000(h: TestHelper, dbh: ODBCDbc) ? =>
    ...
    sth.directexec("CREATE TEMPORARY TABLE t_max_select (a INT, b VARCHAR(30))")?
    ...
    sth.directexec("SELECT * FROM t_max_select")?
  ...
  fun simple_2(h: TestHelper, dbh: ODBCDbc) ? =>
    ...
    sth.directexec("CREATE TEMPORARY TABLE t_myodbc (a INT, b VARCHAR(30))")?

    sth.finish()?

    sth.directexec("INSERT INTO t_myodbc VALUES (10, 'direct')")?
    ...
    sth.directexec("SELECT * FROM t_myodbc")?
