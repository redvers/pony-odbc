...
class \nodoc\ iso _TestTypeTests is UnitTest
  ...
  fun populate_temp_table(h: TestHelper, dbc: ODBCDbc) =>
    var pina: (SQLSmallInteger, SQLInteger, SQLBigInteger, SQLFloat, SQLReal,
              SQLVarchar, SQLVarbinary) =
                (SQLSmallInteger, SQLInteger,
                SQLBigInteger, SQLFloat, SQLReal,
                SQLVarchar(100), SQLVarbinary(100))
    try
      var stm: ODBCStmt = dbc.stmt()?
      stm
  ...
  fun query_test_rowcounts(h: TestHelper, dbc: ODBCDbc) =>
    ...
    var poutb: (SQLSmallInteger, SQLInteger, SQLBigInteger, SQLFloat, SQLReal,
                SQLVarchar, SQLVarbinary) =
                  (SQLSmallInteger, SQLInteger, SQLBigInteger, SQLFloat,
                  SQLReal, SQLVarchar(100), SQLVarbinary(100))
    try
      var stm: ODBCStmt = dbc.stmt()?
  ...
  fun query_test_under_allocation(h: TestHelper, dbc: ODBCDbc) =>
    ...
    var poutb: (SQLSmallInteger, SQLInteger, SQLBigInteger, SQLFloat,
                SQLReal, SQLVarchar, SQLVarbinary) =
                  (SQLSmallInteger, SQLInteger, SQLBigInteger, SQLFloat,
                  SQLReal, SQLVarchar(100), SQLVarbinary(100))
    try
      var stm: ODBCStmt = dbc.stmt()?
